...
export function CartDrawer() {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-surface">
        <SheetHeader>
          <SheetTitle>Carrinho</SheetTitle>
          <SheetDescription>
            {totalItems === 0 ? "O seu carrinho está vazio" : `${totalItems} ${totalItems !== 1 ? "artigos" : "artigo"}`}
          </SheetDescription>
        </SheetHeader>
          ...
          {items.length === 0 ? (
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0 space-y-4">
                {items.map((item) => (
                  <div key={item.variantId} className="flex gap-4 p-3 bg-background rounded-md border border-border/60">
